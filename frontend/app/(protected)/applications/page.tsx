...
import { getAllApplications, getMyApplications, updateApplication, type Application } from "@/lib/api/application.api";
...
export default function ApplicationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const applicationRequest = user.role === "admin" ? getAllApplications(1, 100) : getMyApplications();
      const [applicationResult, universityResult, userResult] = await Promise.all([
        applicationRequest,
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-3xl font-bold tracking-tight text-[#0F172A]">Applications</h1><p className="mt-1 text-sm text-[#64748B]">Live application records from the application service.</p></div>{user.role !== "admin" && <Button onClick={() => router.push("/universities")}>Start application</Button>}</div>
    ...
    <div className="overflow-x-auto rounded-[20px] border border-[#E7EDF6] bg-white shadow-sm"><table className="min-w-[1050px] w-full"><thead><tr className="border-b bg-[#F8FAFC]">{["Student", "University", "Program", "Status", "Stage", "Updated", "Actions"].map((heading) => <th key={heading} className="px-5 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#64748B]">{heading}</th>)}</tr></thead><tbody className="divide-y divide-[#E7EDF6]">
      {filtered.map((item) => <tr key={item.id} className="hover:bg-[#F8FAFC]"><td className="px-5 py-4"><p className="text-sm font-semibold text-[#0F172A]">{userNames.get(item.studentId) || (item.studentId === user.id ? user.fullName : "Unknown user")}</p><p className="text-xs text-[#94A3B8]">{item.studentId.slice(-8)}</p></td><td className="px-5 py-4 text-sm text-[#0F172A]">{universityNames.get(item.universityId) || "Unknown university"}</td><td className="px-5 py-4 text-sm text-[#64748B]">{item.program}</td><td className="px-5 py-4">{user.role === "admin" ? <select disabled={savingId === item.id} value={item.status} onChange={(event) => void changeApplication(item.id, "status", event.target.value)} className="h-9 rounded-lg border px-2 text-xs">{statuses.map((value) => <option key={value} value={value}>{pretty(value)}</option>)}</select> : <Badge variant={statusVariant(item.status)}>{pretty(item.status)}</Badge>}</td><td className="px-5 py-4">{user.role === "admin" ? <select disabled={savingId === item.id} value={item.stage} onChange={(event) => void changeApplication(item.id, "stage", event.target.value)} className="h-9 rounded-lg border px-2 text-xs">{stages.map((value) => <option key={value} value={value}>{pretty(value)}</option>)}</select> : <span className="text-sm text-[#64748B]">{pretty(item.stage)}</span>}</td><td className="px-5 py-4 text-sm text-[#64748B]">{new Date(item.updatedAt).toLocaleDateString()}</td><td className="px-5 py-4"><Button variant="ghost" size="sm" onClick={() => router.push(`/applications?selected=${item.id}`)}><Send className="h-4 w-4"/>Details</Button></td></tr>)}
      {filtered.length === 0 && <tr><td colSpan={7} className="px-6 py-16 text-center text-sm text-[#64748B]">No application records match this view.</td></tr>}
    </tbody></table></div>
  </div>;
}
